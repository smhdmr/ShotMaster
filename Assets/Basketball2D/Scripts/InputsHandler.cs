			...
			if (objname == btn.name) {
				if (!btnComp.isBegan) {
					...
					if (btnComp.clickReleaseSFx != null) {
						if(SFX.instance.audioSources.Length !=0){
							SFX.instance.audioSources [0].clip = btnComp.clickReleaseSFx;//play sfx on click or on touch began
							AudioManager.PlayClip (SFX.instance.audioSources [0]);
						}
					}
				}
			} else {
